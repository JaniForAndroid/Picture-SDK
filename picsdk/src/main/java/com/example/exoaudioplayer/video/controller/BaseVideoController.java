...
import com.example.exoaudioplayer.video.util.OrientationHelper;
...
public abstract class BaseVideoController extends FrameLayout
  ...
  public void setMediaPlayer(IMediaPlayerControl mediaPlayer) {
    mControlWrapper = new ControlWrapperI(mediaPlayer, this);
    //绑定ControlComponent和Controller
    for (Entry<IControlComponent, Boolean> next : mControlComponents.entrySet()) {
  ...
  public void removeAllPrivateComponents() {
    Iterator<Entry<IControlComponent, Boolean>> it = mControlComponents.entrySet().iterator();
    while (it.hasNext()) {
      Entry<IControlComponent, Boolean> next = it.next();
      if (next.getValue()) {
        it.remove();
      }
    }
...
//        L.d("hasCutout: " + mHasCutout + " cutout height: " + mCutoutHeight);
  ...
  protected void onOrientationLandscape(Activity activity) {
    ...
    if (mControlWrapper.isFullScreen()) {
      handlePlayerStateChanged(Constants.PLAYER_FULL_SCREEN);
    } else {
      mControlWrapper.startFullScreen();
    }
  }
  ...
  private void handleVisibilityChanged(boolean isVisible, Animation anim) {
    if (!mIsLocked) { //没锁住时才向ControlComponent下发此事件
      for (Entry<IControlComponent, Boolean> next
