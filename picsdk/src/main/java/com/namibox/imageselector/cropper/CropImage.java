 ...
 * intents.<br> The goal of the helper is to simplify the starting and most-common usage of image
 * cropping and not all porpose all possible scenario one-to-rule-them-all code base. So feel free
 * to use it as is and as a wiki to make your own.<br> Added value you get out-of-the-box is some
 * edge case handling that you may miss otherwise, like the stupid-ass Android camera result URI
 * that may differ from version to version and from device to device.
 */
public final class CropImage {
  ...
  public static void startPickImageActivity(Activity activity) {
    activity.startActivityForResult(getPickImageChooserIntent(activity),
   ...
   */
  public static Intent getPickImageChooserIntent(Context context, CharSequence title,
  ...
  /**
   * Check if the given picked image URI requires READ_EXTERNAL_STORAGE permissions.<br> Only
   * relevant for API version 23 and above and not required for all URI's depends on the
   ...
   * image.<br> Result will be recieved in {@link Activity#onActivityResult(int, int, Intent)} and
   * can be retrieved using {@link #getActivityResult(Intent)}.
   *
  ...
  public static final class ActivityBuilder {
     ...
     * Dest Activity
     */
    private Class mDestActivityClass;
    ...
    public ActivityBuilder setDestActivityClass(Class destActivityClass) {
      mDestActivityClass = destActivityClass;
      return this;
    }
    ...
    public Intent getIntent(Context context) {
      ...
      if (mDestActivityClass != null) {
        intent.setClass(context, mDestActivityClass);
      } else {
    ...
    /**
     * the size to downsample the cropped image to.<br> NOTE: resulting image will not be exactly
     * (reqWidth, reqHeight) see: <a href="http://developer.android.com/training/displaying-bitmaps/load-bitmap.html">Loading
     * Large Bitmaps Efficiently</a><br>
     */
    ...
    /**
     * if the result of crop image activity should not save the cropped image bitmap.<br> Used if
     * you want to crop the image manually and need only the crop rectangle and rotation data.
     */
