...
public class CropImageView extends FrameLayout {
  ...
  public CropImageView(Context context, AttributeSet attrs) {
    ...
    if (options == null) {
      ...
      if (attrs != null) {
        ...
        try {
          options.fixAspectRatio = ta
              .getBoolean(R.styleable.CropImageView_cropFixAspectRatio, options.fixAspectRatio);
          options.aspectRatioX = ta
              .getInteger(R.styleable.CropImageView_cropAspectRatioX, options.aspectRatioX);
          options.aspectRatioY = ta
              .getInteger(R.styleable.CropImageView_cropAspectRatioY, options.aspectRatioY);
          options.scaleType = ScaleType.values()[ta
              .getInt(R.styleable.CropImageView_cropScaleType, options.scaleType.ordinal())];
          options.autoZoomEnabled = ta
              .getBoolean(R.styleable.CropImageView_cropAutoZoomEnabled, options.autoZoomEnabled);
          options.maxZoom = ta.getInteger(R.styleable.CropImageView_cropMaxZoom, options.maxZoom);
          options.cropShape = CropShape.values()[ta
              .getInt(R.styleable.CropImageView_cropShape, options.cropShape.ordinal())];
          options.guidelines = Guidelines.values()[ta
              .getInt(R.styleable.CropImageView_cropGuidelines, options.guidelines.ordinal())];
          options.snapRadius = ta
              .getDimension(R.styleable.CropImageView_cropSnapRadius, options.snapRadius);
          options.touchRadius = ta
              .getDimension(R.styleable.CropImageView_cropTouchRadius, options.touchRadius);
          options.initialCropWindowPaddingRatio = ta
              .getFloat(R.styleable.CropImageView_cropInitialCropWindowPaddingRatio,
                  options.initialCropWindowPaddingRatio);
          options.borderLineThickness = ta
              .getDimension(R.styleable.CropImageView_cropBorderLineThickness,
                  options.borderLineThickness);
          options.borderLineColor = ta
              .getInteger(R.styleable.CropImageView_cropBorderLineColor, options.borderLineColor);
          options.borderCornerThickness = ta
              .getDimension(R.styleable.CropImageView_cropBorderCornerThickness,
                  options.borderCornerThickness);
          options.borderCornerOffset = ta
              .getDimension(R.styleable.CropImageView_cropBorderCornerOffset,
                  options.borderCornerOffset);
          options.borderCornerLength = ta
              .getDimension(R.styleable.CropImageView_cropBorderCornerLength,
                  options.borderCornerLength);
          options.borderCornerColor = ta.getInteger(R.styleable.CropImageView_cropBorderCornerColor,
              options.borderCornerColor);
          options.guidelinesThickness = ta
              .getDimension(R.styleable.CropImageView_cropGuidelinesThickness,
                  options.guidelinesThickness);
          options.guidelinesColor = ta
              .getInteger(R.styleable.CropImageView_cropGuidelinesColor, options.guidelinesColor);
          options.backgroundColor = ta
              .getInteger(R.styleable.CropImageView_cropBackgroundColor, options.backgroundColor);
          options.showCropOverlay = ta
              .getBoolean(R.styleable.CropImageView_cropShowCropOverlay, mShowCropOverlay);
          options.showProgressBar = ta
              .getBoolean(R.styleable.CropImageView_cropShowProgressBar, mShowProgressBar);
          options.borderCornerThickness = ta
              .getDimension(R.styleable.CropImageView_cropBorderCornerThickness,
                  options.borderCornerThickness);
          options.minCropWindowWidth = (int) ta
              .getDimension(R.styleable.CropImageView_cropMinCropWindowWidth,
                  options.minCropWindowWidth);
          options.minCropWindowHeight = (int) ta
              .getDimension(R.styleable.CropImageView_cropMinCropWindowHeight,
                  options.minCropWindowHeight);
          options.minCropResultWidth = (int) ta
              .getFloat(R.styleable.CropImageView_cropMinCropResultWidthPX,
                  options.minCropResultWidth);
          options.minCropResultHeight = (int) ta
              .getFloat(R.styleable.CropImageView_cropMinCropResultHeightPX,
                  options.minCropResultHeight);
          options.maxCropResultWidth = (int) ta
              .getFloat(R.styleable.CropImageView_cropMaxCropResultWidthPX,
                  options.maxCropResultWidth);
          options.maxCropResultHeight = (int) ta
              .getFloat(R.styleable.CropImageView_cropMaxCropResultHeightPX,
  ...
  /**
   * Gets the cropped image based on the current crop window.<br> If image loaded from URI will use
   * sample size to fit in the requested width and height down-sampling if required - optimization
   * to get best size to quality.<br> NOTE: resulting image will not be exactly (reqWidth,
   * reqHeight) see: <a href="http://developer.android.com/training/displaying-bitmaps/load-bitmap.html">Loading
   ...
   * listener set by {@link #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
   */
   ...
   * AND image is loaded from URI cropping will try to use sample size to fit in the requested width
   * and height down-sampling if possible - optimization to get best size to quality.<br> NOTE:
   * resulting image will not be exactly (reqWidth, reqHeight) see: <a
   * href="http://developer.android.com/training/displaying-bitmaps/load-bitmap.html">Loading Large
   * Bitmaps Efficiently</a><br> The result will be invoked to listener set by {@link
   * #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
   *
   * @param reqWidth the width to downsample the cropped image to
   * @param reqHeight the height to downsample the cropped image to
   */
   ...
   * be invoked to listener set by {@link #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
   *
   * @param saveUri the Android Uri to save the cropped image to
   * @param saveCompressFormat the compression format to use when writting the image
   * @param saveCompressQuality the quility (if applicable) to use when writting the image (0 -
   * 100)
   */
  public void saveCroppedImageAsync(Uri saveUri, Bitmap.CompressFormat saveCompressFormat,
      int saveCompressQuality) {
    saveCroppedImageAsync(saveUri, saveCompressFormat, saveCompressQuality, 0, 0);
  }

  /**
   * Save the cropped image based on the current crop window to the given uri.<br> If
   * (reqWidth,reqHeight) is given AND image is loaded from URI cropping will try to use sample size
   * to fit in the requested width and height down-sampling if possible - optimization to get best
   * size to quality.<br> NOTE: resulting image will not be exactly (reqWidth, reqHeight) see: <a
   * href="http://developer.android.com/training/displaying-bitmaps/load-bitmap.html">Loading Large
   * Bitmaps Efficiently</a><br> The result will be invoked to listener set by {@link
   * #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
   *
  ...
  /**
   * Set the callback to be invoked when image async get cropping image ({@link
   * #getCroppedImageAsync()}) is complete (successful or failed).
   */
  ...
  /**
   * Set the callback to be invoked when image async save cropping image ({@link
   * #saveCroppedImageAsync(Uri)}) is complete (successful or failed).
   */
  ...
  public void rotateImage(int degrees) {
    if (mBitmap != null) {
      if (degrees % 90 == 0) {
        ...
        mDegreesRotated =
            mDegreesRotated >= 0 ? mDegreesRotated % 360 : mDegreesRotated % 360 + 360;
  ...
  /**
   * Set the given bitmap to be used in for cropping<br> Optionally clear full if the bitmap is new,
   * or partial clear if the bitmap has been manipulated.
   */
   ...
   * Gets the cropped image based on the current crop window.<br> If (reqWidth,reqHeight) is given
   * AND image is loaded from URI cropping will try to use sample size to fit in the requested width
   * and height down-sampling if possible - optimization to get best size to quality.<br> The result
   * will be invoked to listener set by {@link #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
   *
   ...
   * the image
   * @param saveCompressQuality if saveUri is given, the given quiality will be used for the
   * compression.
   */
  public void startCropWorkerTask(int reqWidth, int reqHeight, Uri saveUri,
    ...
    if (mBitmap != null) {
      ...
      if (mLoadedImageUri != null && mLoadedSampleSize > 1) {
        mBitmapCroppingWorkerTask = new WeakReference<>(
            new BitmapCroppingWorkerTask(this, mLoadedImageUri, getCropPoints(),
                mDegreesRotated, orgWidth, orgHeight,
                mCropOverlayView.isFixAspectRatio(), mCropOverlayView.getAspectRatioX(),
                mCropOverlayView.getAspectRatioY(),
        ...
        mBitmapCroppingWorkerTask = new WeakReference<>(
            new BitmapCroppingWorkerTask(this, mBitmap, getCropPoints(), mDegreesRotated,
                mCropOverlayView.isFixAspectRatio(), mCropOverlayView.getAspectRatioX(),
                mCropOverlayView.getAspectRatioY(), saveUri, saveCompressFormat,
                saveCompressQuality));
      }
  ...
  /**
   * Handle crop window change to:<br> 1. Execute auto-zoom-in/out depending on the area covered of
   * cropping window relative to the available view area.<br> 2. Slide the zoomed sub-area if the
   * cropping window is outside of the visible view sub-area.<br>
   *
    ...
    if (mBitmap != null && width > 0 && height > 0) {
      ...
      if (inProgress) {
        ...
        if (mZoom < mMaxZoom && cropRect.width() < width * 0.5f
          ...
          newZoom = Math.min(mMaxZoom, Math.min(width / (cropRect.width() / mZoom / 0.64f),
  ...
  /**
   * Adjust the given crop window rectangle by the change in zoom, need to update the location and
   * size of the crop rectangle to cover the same area in new zoom level.
   */
  ...
  private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
    if (mBitmap != null && width > 0 && height > 0) {
      ...
      mImageMatrix
          .postTranslate((width - mImageRect.width()) / 2, (height - mImageRect.height()) / 2);
      ...
      if (center) {
        ...
        mZoomOffsetX = width > mImageRect.width() ? 0
            : Math.max(Math.min(width / 2 - cropRect.centerX(), -mImageRect.left),
                getWidth() - mImageRect.right) / mZoom;
        mZoomOffsetY = height > mImageRect.height() ? 0
            : Math.max(Math.min(height / 2 - cropRect.centerY(), -mImageRect.top),
                getHeight() - mImageRect.bottom) / mZoom;
      } else {
        // adjust the zoomed area so the crop window rectangle will be inside the area in case it was moved outside
        mZoomOffsetX =
            Math.min(Math.max(mZoomOffsetX * mZoom, -cropRect.left), -cropRect.right + width)
                / mZoom;
        mZoomOffsetY =
            Math.min(Math.max(mZoomOffsetY * mZoom, -cropRect.top), -cropRect.bottom + height)
                / mZoom;
      }
   ...
   * the image.<br> To get the proper rectangle it must be first reset to orginal image rectangle.
   */
  ...
  private void updateBitmapRect(RectF bitmapRect) {
    if (mBitmap != null && bitmapRect.width() > 0 && bitmapRect.height() > 0) {
      ...
      mCropOverlayView
          .setCropWindowLimits(getWidth(), getHeight(), scaleFactorWidth, scaleFactorHeight);
    }
  ...
  public enum ScaleType {
    ...
    /**
     * Center the image in the view, but perform no scaling.<br> Note: If auto-zoom is enabled and
     * the source image is smaller than crop image view then it will be scaled uniformly to fit the
     * crop image view.
     */
    CENTER,
     ...
     * the view (minus padding).<br> The image is then centered in the view.<br> Note: If auto-zoom
     * is enabled and the source image is smaller than crop image view then it will be scaled
     * uniformly to fit the crop image view.
     */
    CENTER_INSIDE
  }
  //endregion
