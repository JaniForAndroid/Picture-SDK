...
/**
 * Handler to update crop window edges by the move type - Horizontal, Vertical, Corner or
 * Center.<br/>
 */
final class CropWindowMoveHandler {
   ...
   */
  public CropWindowMoveHandler(Type type, CropWindowHandler cropWindowHandler, float touchX,
      float touchY) {
    mType = type;
  ...
  /**
   * Updates the crop window by change in the toch location.<br> Move type handled by this instance,
   * as initialized in creation, affects how the change in toch location changes the crop window
   * position and size.<br> After the crop window position/size is changed by toch move it may
   * result in values that vialate contraints: outside the bounds of the shown bitmap,
   * smaller/larger than min/max size or missmatch in aspect ratio. So a series of fixes is executed
   * on "secondary" edges to adjust it by the "primary" edge movement.<br> Primary is the edge
   * directly affected by move type, secondary is the other edge.<br> The crop window is changed by
   * directly setting the Edge coordinates.
   *
   ...
   */
  public void move(float x, float y, RectF bounds, int viewWidth, int viewHeight, float snapMargin,
      boolean fixedAspectRatio, float aspectRatio) {
    ...
    if (mType == Type.CENTER) {
      ...
      if (fixedAspectRatio) {
        moveSizeWithFixedAspectRatio(adjX, adjY, bounds, viewWidth, viewHeight, snapMargin,
            aspectRatio);
      } else {
   ...
   */
  private void moveCenter(float x, float y, RectF bounds, int viewWidth, int viewHeight,
      float snapRadius) {
    RectF rect = mCropWindowHandler.getRect();
   ...
   */
  private void moveSizeWithFreeAspectRatio(float x, float y, RectF bounds, int viewWidth,
      int viewHeight, float snapMargin) {
    switch (mType) {
      case TOP_LEFT:
   ...
   * "secondary" edge via aspect ratio.<br> Example: change in the left edge (primary) will affect
   * top and bottom edges (secondary) to preserve the given aspect ratio.
   */
  private void moveSizeWithFixedAspectRatio(float x, float y, RectF bounds, int viewWidth,
   ...
   */
  private void adjustLeft(float left, RectF bounds, float snapMargin, float aspectRatio,
      boolean topMoves, boolean bottomMoves) {
    ...
    if (aspectRatio > 0) {
      ...
      if (newHeight < mCropWindowHandler.getMinCropHeight()) {
        newLeft = Math
            .max(bounds.left, rect.right - mCropWindowHandler.getMinCropHeight() * aspectRatio);
      ...
      if (newHeight > mCropWindowHandler.getMaxCropHeight()) {
        newLeft = Math
            .max(bounds.left, rect.right - mCropWindowHandler.getMaxCropHeight() * aspectRatio);
        newHeight = (rect.right - newLeft) / aspectRatio;
      }
      ...
      if (topMoves && bottomMoves) {
        newLeft = Math
            .max(newLeft, Math.max(bounds.left, rect.right - bounds.height() * aspectRatio));
      } else {
   ...
   * @param snapMargin the snap distance to the image edge (in pixels)
   */
  private void adjustRight(float right, RectF bounds, int viewWidth, float snapMargin,
    ...
    if (aspectRatio > 0) {
      ...
      if (newHeight < mCropWindowHandler.getMinCropHeight()) {
        newRight = Math
            .min(bounds.right, rect.left + mCropWindowHandler.getMinCropHeight() * aspectRatio);
      ...
      if (newHeight > mCropWindowHandler.getMaxCropHeight()) {
        newRight = Math
            .min(bounds.right, rect.left + mCropWindowHandler.getMaxCropHeight() * aspectRatio);
        newHeight = (newRight - rect.left) / aspectRatio;
      }
      ...
      if (topMoves && bottomMoves) {
        newRight = Math
            .min(newRight, Math.min(bounds.right, rect.left + bounds.height() * aspectRatio));
      } else {
        // if top edge moves by aspect ratio check that it is within bounds
        if (topMoves && rect.bottom - newHeight < bounds.top) {
   ...
   * @param snapMargin the snap distance to the image edge (in pixels)
   */
  private void adjustTop(float top, RectF bounds, float snapMargin, float aspectRatio,
    ...
    if (aspectRatio > 0) {
      ...
      if (newWidth < mCropWindowHandler.getMinCropWidth()) {
        newTop = Math
            .max(bounds.top, rect.bottom - (mCropWindowHandler.getMinCropWidth() / aspectRatio));
      ...
      if (newWidth > mCropWindowHandler.getMaxCropWidth()) {
        newTop = Math
            .max(bounds.top, rect.bottom - (mCropWindowHandler.getMaxCropWidth() / aspectRatio));
        newWidth = (rect.bottom - newTop) * aspectRatio;
      }
   ...
   * @param snapMargin the snap distance to the image edge (in pixels)
   */
  private void adjustBottom(float bottom, RectF bounds, int viewHeight, float snapMargin,
    ...
    if (aspectRatio > 0) {
      ...
      if (newWidth < mCropWindowHandler.getMinCropWidth()) {
        newBottom = Math
            .min(bounds.bottom, rect.top + mCropWindowHandler.getMinCropWidth() / aspectRatio);
      ...
      if (newWidth > mCropWindowHandler.getMaxCropWidth()) {
        newBottom = Math
            .min(bounds.bottom, rect.top + mCropWindowHandler.getMaxCropWidth() / aspectRatio);
        newWidth = (newBottom - rect.top) * aspectRatio;
      }
      ...
      if (leftMoves && rightMoves) {
        newBottom = Math
            .min(newBottom, Math.min(bounds.bottom, rect.top + bounds.width() / aspectRatio));
      } else {
        // if left edge moves by aspect ratio check that it is within bounds
        if (leftMoves && rect.right - newWidth < bounds.left) {
