...
import com.namibox.simplifyspan.customspan.CustomClickableSpan;
import com.namibox.simplifyspan.customspan.CustomImageSpan;
import com.namibox.simplifyspan.customspan.CustomLabelSpan;
import com.namibox.simplifyspan.other.CustomLinkMovementMethod;
...
import com.namibox.simplifyspan.unit.SpecialClickableUnit;
import com.namibox.simplifyspan.unit.SpecialImageUnit;
import com.namibox.simplifyspan.unit.SpecialLabelUnit;
import com.namibox.simplifyspan.unit.SpecialRawSpanUnit;
...
public class SimplifySpanBuild {
  ...
  public SimplifySpanBuild(Context context, TextView textView, String initializeNormalText,
      BaseSpecialUnit... normalSpecialUnits) {
    init(context, textView, initializeNormalText, normalSpecialUnits);
  }

  private void init(Context context, TextView textView, String initializeNormalText,
      BaseSpecialUnit... normalSpecialUnits) {
    mStringBuilder = new StringBuilder(
        TextUtils.isEmpty(initializeNormalText) ? "" : initializeNormalText);
  ...
  private void buildNormalSpecialUnits(boolean isAddToBeforeSpecialUnit, int initStartPos,
      String finalText, BaseSpecialUnit... normalSpecialUnits) {
    ...
    for (BaseSpecialUnit st : normalSpecialUnits) {
      ...
      switch (st.getConvertMode()) {
        ...
        case SpecialConvertMode.ALL:
          ...
          while (hasNext) {
            int nextSpecialTextStartPos = finalText
                .indexOf(specialText, previousSpecialTextStartPos);
      ...
      if (null == startPossTemp || startPossTemp.length == 0) {
        continue;
      }
   ...
   * @return SimplifySpanBuild
   */
  public SimplifySpanBuild appendNormalTextToFirst(String text,
      BaseSpecialUnit... normalSpecialUnits) {
    if (TextUtils.isEmpty(text)) {
      return this;
    }
   ...
   */
  public SimplifySpanBuild appendMultiClickableSpecialUnit(
   ...
   * @param specialUnitOrStrings Unit Or String
   */
  public SimplifySpanBuild appendMultiClickableSpecialUnitToFirst(
      SpecialClickableUnit specialClickableUnit, Object... specialUnitOrStrings) {
  ...
  public SpannableStringBuilder build() {
    if (mBeforeStringBuilder.length() > 0) {
      ...
      if (!mFinalSpecialUnit.isEmpty()) {
        for (BaseSpecialUnit specialUnit : mFinalSpecialUnit) {
          ...
          if (null == tempStartPoss || tempStartPoss.length == 0) {
            continue;
          }
      ...
      if (!mCacheSpecialClickableUnitMap.isEmpty()) {
        for (Map.Entry<SpecialClickableUnit, PositionInfo> cm : mCacheSpecialClickableUnitMap
            .entrySet()) {
    ...
    for (BaseSpecialUnit st : mFinalSpecialUnit) {
      ...
      if (st instanceof SpecialTextUnit) {
        ...
        final SpecialClickableUnit internalSpecialClickableUnit = specialTextUnit
            .getSpecialClickableUnit();
        if (null != internalSpecialClickableUnit) {
          ...
          if (internalSpecialClickableUnit.getNormalBgColor() == 0) {
            internalSpecialClickableUnit
                .setNormalBgColor(specialTextUnit.getSpecialTextBackgroundColor());
          }
        }
        for (int startPos : startPoss) {
          ...
          if (specialTextUnit.getSpecialTextColor() != 0) {
            spannableStringBuilder
                .setSpan(new ForegroundColorSpan(specialTextUnit.getSpecialTextColor()), startPos,
                    startPos + specialTextLength, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }

          // Set Text Background Color
          if (specialTextUnit.getSpecialTextBackgroundColor() != 0
              && null == internalSpecialClickableUnit) {
            spannableStringBuilder
                .setSpan(new BackgroundColorSpan(specialTextUnit.getSpecialTextBackgroundColor()),
                    startPos, startPos + specialTextLength, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }

          // Add Underline
          if (specialTextUnit.isShowUnderline()) {
            spannableStringBuilder
                .setSpan(new UnderlineSpan(), startPos, startPos + specialTextLength,
          ...
          if (specialTextUnit.isShowStrikeThrough()) {
            spannableStringBuilder
                .setSpan(new StrikethroughSpan(), startPos, startPos + specialTextLength,
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }
          ...
          if (specialTextUnit.isTextBold()) {
            spannableStringBuilder
                .setSpan(new StyleSpan(Typeface.BOLD), startPos, startPos + specialTextLength,
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }
          ...
          if (specialTextUnit.getTextSize() > 0) {
            spannableStringBuilder.setSpan(
                new CustomAbsoluteSizeSpan(normalSizeText, specialTextUnit.getSpecialText(),
                    Math.round(specialTextUnit.getTextSize()), mTextView,
                    specialTextUnit.getGravity()), startPos, startPos + specialTextLength,
                Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }
          ...
          if (null != internalSpecialClickableUnit) {
            ...
            spannableStringBuilder
                .setSpan(new CustomClickableSpan(internalSpecialClickableUnit), startPos,
                    startPos + specialTextLength, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
          }
        }
        ...
        for (int startPos : startPoss) {
          CustomImageSpan curCustomImageSpan = new CustomImageSpan(mContext, normalSizeText,
              specialImageUnit);
          spannableStringBuilder.setSpan(curCustomImageSpan, startPos, startPos + specialTextLength,
              Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

          if (specialImageUnit.isClickable()) {
            addClickStateChangeListeners(startPos, startPos + specialTextLength,
                curCustomImageSpan);
          }
        }
        ...
        for (int startPos : startPoss) {
          CustomLabelSpan curCustomLabelSpan = new CustomLabelSpan(normalSizeText,
              specialLabelUnit);
          spannableStringBuilder.setSpan(curCustomLabelSpan, startPos, startPos + specialTextLength,
              Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        spannableStringBuilder.setSpan(new CustomClickableSpan(specialClickableUnit), startPos,
            startPos + specialTextLength, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        int startPos = startPoss[0];
        spannableStringBuilder
            .setSpan(specialRawSpanUnit.getSpanObj(), startPos, startPos + specialTextLength,
                specialRawSpanUnit.getFlags());
  ...
  private void processMultiClickableSpecialUnit(boolean isToFirst,
      SpecialClickableUnit specialClickableUnit, Object... specialUnitOrStrings) {
    if (null == specialClickableUnit || null == specialUnitOrStrings
        || specialUnitOrStrings.length == 0) {
      return;
    }
  ...
  private void addClickStateChangeListeners(int startPos, int endPos,
      OnClickStateChangeListener onClickStateChangeListener) {
    if (mCacheSpecialClickableUnitMap.isEmpty()) {
      return;
    }
    ...
    for (Map.Entry<SpecialClickableUnit, PositionInfo> cm : mCacheSpecialClickableUnitMap
      ...
      if (startPos >= curStartPos && endPos <= curEndPos) {
        ...
        List<OnClickStateChangeListener> onClickStateChangeListeners = curCacheSpecialClickableUnit
            .getOnClickStateChangeListeners();
