package com.namibox.simplifyspan.customspan;
...
public class CustomLabelSpan extends ReplacementSpan implements OnClickStateChangeListener {
    ...
    if (isInit) {
      ...
      if (isDrawBitmap) {
        Bitmap newBitmap = ThumbnailUtils
            .extractThumbnail(mBitmap, Math.round(mFinalWidth), Math.round(mFinalHeight));
        if (null != newBitmap) {
          mBitmap.recycle();
          mBitmap = newBitmap;
        }
  ...
  public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y,
    ...
    switch (mSpecialLabelUnit.getGravity()) {
      case SpecialGravity.TOP:
        ...
        newTextY -= (mLineTextHeight - mSpecialTextHeight - (mLineTextBaselineOffset
            - mSpecialTextBaselineOffset) - mPaddingTop);
        break;
      case SpecialGravity.CENTER:
        newStartY -= (mLineTextHeight / 2 + mFinalHeight / 2 - mLineTextBaselineOffset);
        newTextY -= (mLineTextHeight / 2 - mSpecialTextHeight / 2 - (mLineTextBaselineOffset
            - mSpecialTextBaselineOffset));
        break;
  ...
  private float initFinalHeight(Paint paint) {
    if (mFinalHeight <= 0) {
      ...
      if (labelBgHeight > 0 && labelBgHeight > mSpecialTextHeight
          && labelBgHeight <= mLineTextHeight) {
