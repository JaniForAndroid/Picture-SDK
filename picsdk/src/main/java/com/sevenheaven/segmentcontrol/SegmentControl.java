    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mTexts != null && mTexts.length > 0) {
            ...
            for (int i = 0; i < mTexts.length; i++) {
                ...
                //draw texts
                float textHeight = mCachedFM.descent - mCachedFM.ascent;
                float baseline = mCacheBounds[i].top + ((mSingleChildHeight + textHeight) / 2) - mCachedFM.descent;
                canvas.drawText(mTexts[i], mCacheBounds[i].left + (mSingleChildWidth - mTextBounds[i].width()) / 2, baseline, mPaint);
                if (mNumbers != null && i < mNumbers.length && mNumbers[i] > 0) {
                    textHeight = textHeight * 0.8f;
                    mNumberPaint.setTextSize(mTextSize * 0.7f);
                    String number = mNumbers[i] > 9 ? "9+" : String.valueOf(mNumbers[i]);
                    float y = mCacheBounds[i].top + textHeight - mCachedFM.descent;
                    float radius = textHeight / 2;
                    float cx  = mCacheBounds[i].right - radius;
                    float cy  = mCacheBounds[i].top + radius;
                    mNumberPaint.setColor(Color.RED);
                    canvas.drawCircle(cx, cy, radius, mNumberPaint);
                    mNumberPaint.setColor(Color.WHITE);
                    canvas.drawText(number, cx, y, mNumberPaint);
                }
            }
        }
    }
