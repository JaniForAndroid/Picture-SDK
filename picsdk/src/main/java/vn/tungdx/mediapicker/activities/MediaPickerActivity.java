...
public class MediaPickerActivity extends AppCompatActivity implements
  ...
  public static void open(Activity activity, MediaOptions options,
      String videoId) {
    Intent intent = new Intent(activity, MediaPickerActivity.class);
    intent.putExtra(EXTRA_MEDIA_OPTIONS, options);
//        intent.putExtra(ISRECORD, isRecord);
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    no_actionbar = getIntent().getBooleanExtra("no_actionbar", false);
...
//        }
    ...
    if (getSupportActionBar() != null) {
      getSupportActionBar().setBackgroundDrawable(
          ContextCompat.getDrawable(this, R.drawable.picker_actionbar_translucent));
...
//                }
  ...
  private void returnBackData(List<MediaItem> mediaSelectedList) {
    ...
    if (mMediaOptions.isOpenOther()) {
      Intent data = new Intent(this, mMediaOptions.getCls());
   ...
   *
   * @return 1 if valid, otherwise is invalid. -2: not found, 0 larger than accepted, -1 smaller
   * than accepted.
   */
   ...
   *
   * @param intent In {@link Activity#onActivityResult(int, int, Intent)} method of activity or
   * fragment that open media picker.
   * @return Always return {@link ArrayList} of {@link MediaItem}. You must always check null and
   * size of this list before handle your logic.
   */
